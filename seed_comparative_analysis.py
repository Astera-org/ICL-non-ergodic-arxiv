...
def create_validation_dataloader(seed, multiplier=1100):
    """Create a validation dataloader for k=1 with the specified seed."""
    try:
...
def compute_model_position_loss(model, dataloader, run_name, device='cpu'):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"Computing loss for {run_name}", leave=True):
...
def plot_seed_comparison(seed, results_by_k):
    ...
    plots_dir = f"seed_comparison_plots"
    ...
    for k, result in results_by_k.items():
        ...
        plt.plot(positions, losses, '-', color=colors.get(k, 'gray'), 
                 linewidth=1.5, label=f'k={k} (mean={mean_loss:.4f})')
    
    # Add grid and labels
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.xlabel('Position in Context Window', fontsize=12)
    plt.ylabel('Average Loss', fontsize=12)
    plt.title(f'Loss by Position for Different k Values (Seed {seed})', fontsize=14)
    ...
    plt.xlabel('Position in Context Window', fontsize=12)
    plt.ylabel('Average Loss', fontsize=12)
    plt.title(f'Loss by Position for Different k Values (Seed {seed}, First 50 Positions)', fontsize=14)
...
def main():
    ...
    for seed, runs in runs_by_seed.items():
        ...
        dataloader = create_validation_dataloader(seed)
